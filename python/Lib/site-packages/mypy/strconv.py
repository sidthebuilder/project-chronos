...
class StrConv(NodeVisitor[str]):
    ...
    def visit_mypy_file(self, o: mypy.nodes.MypyFile) -> str:
        ...
        if o.ignored_lines:
            a.append(
                "IgnoredLines(%s)" % ", ".join(str(line) for line in sorted(o.ignored_lines))
            )
    ...
    def visit_class_def(self, o: mypy.nodes.ClassDef) -> str:
        ...
        if o.info and o.info._promote:
            a.insert(
                1, f"Promote([{','.join(self.stringify_type(p) for p in o.info._promote)}])"
            )
...
def dump_tagged(nodes: Sequence[object], tag: str | None, str_conv: StrConv) -> str:
    ...
    for n in nodes:
        if isinstance(n, list):
            ...
            a.append(
                indent(
                    n.accept(TypeStrVisitor(str_conv.id_mapper, options=str_conv.options)), 2
                )
            )
        elif n is not None:
