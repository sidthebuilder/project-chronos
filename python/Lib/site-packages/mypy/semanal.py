...
class SemanticAnalyzer(
    ...
    def analyze_func_def(self, defn: FuncDef) -> None:
        ...
        if self.is_class_scope():
            ...
            if (
                ...
                and (
                    not isinstance(self.scope.function, OverloadedFuncDef) or defn.is_property
                )
    ...
    def process_final_in_overload(self, defn: OverloadedFuncDef) -> None:
        ...
        if any(item.is_final for item in defn.items):
            ...
            if not self.is_stub_file:
                self.fail(
                    "@final should be applied only to overload implementation", bad_final
                )
        ...
        if isinstance(decorator, RefExpr):
            if decorator.fullname in RUNTIME_PROTOCOL_DECOS:
                if defn.info.is_protocol:
                    ...
                    self.fail(
                        "@runtime_checkable can only be used with protocol classes", defn
                    )
            ...
            if result is not None:
                ...
                if declared_tvars:
                    if defn.type_args:
                        if is_protocol:
                            self.fail(
                                'No arguments expected for "Protocol" base class', context
                            )
                        else:
    ...
    def configure_tuple_base_class(self, defn: ClassDef, base: TupleType) -> Instance:
        ...
        if base.partial_fallback.type.fullname == "builtins.tuple" and not has_placeholder(
            base
        ):
    ...
    def check_and_set_up_type_alias(self, s: AssignmentStmt) -> bool:
        ...
        if existing and (
            ...
            or (
                isinstance(existing.node, PlaceholderNode) and existing.node.node.line < s.line
            )
    ...
    def check_typevartuple_default(self, default: Type, context: Context) -> Type:
        ...
        if not isinstance(typ, UnpackType):
            self.fail(
                "The default argument to TypeVarTuple must be an Unpacked tuple", context
            )
            default = AnyType(TypeOfAny.from_error)
        return default
    ...
    def check_typevarlike_name(self, call: CallExpr, name: str, context: Context) -> bool:
        ...
        if not isinstance(call.args[0], StrExpr) or call.arg_kinds[0] != ARG_POS:
            self.fail(
                f"{typevarlike_type}() expects a string literal as first argument", context
            )
            return False
        ...
        for param_value, param_name, param_kind in zip(args, names, kinds):
            ...
            if param_name == "covariant":
                if isinstance(param_value, NameExpr) and param_value.name in ("True", "False"):
                    ...
                    self.fail(
                        message_registry.TYPEVAR_VARIANCE_DEF.format("covariant"), context
                    )
                    return None
        ...
        if not s.type.args:
            if isinstance(s.rvalue, TempNode) and s.rvalue.no_rhs:
                if self.options.disallow_any_generics:
                    self.fail(
                        "ClassVar without type argument becomes Any", s, code=codes.TYPE_ARG
                    )
                return
            s.type = None
    ...
    def process__deletable__(self, s: AssignmentStmt) -> None:
        ...
        if (
            ...
            if not isinstance(rvalue, (ListExpr, TupleExpr)):
                self.fail(
                    '"__deletable__" must be initialized with a list or tuple expression', s
                )
                return
            items = rvalue.items
            attrs = []
    ...
    def visit_type_alias_stmt(self, s: TypeAliasStmt) -> None:
        ...
        try:
            ...
            if existing and not (
                ...
                or (
                    isinstance(existing.node, PlaceholderNode) and existing.node.line == s.line
                )
            ):
    ...
    def analyze_type_application_args(self, expr: IndexExpr) -> list[Type] | None:
        ...
        if isinstance(index, TupleExpr):
            items = index.items
            is_tuple = (
                isinstance(expr.base, RefExpr) and expr.base.fullname == "builtins.tuple"
            )
        ...
        if has_param_spec and num_args == 1 and types:
            ...
            if not (
                single_any or any(isinstance(t, (Parameters, ParamSpecType)) for t in types)
            ):
    ...
    def visit_list_comprehension(self, expr: ListComprehension) -> None:
        if any(expr.generator.is_async):
            if (
                ...
                self.fail(
                    message_registry.ASYNC_FOR_OUTSIDE_COROUTINE, expr, code=codes.SYNTAX
                )
    ...
    def visit_await_expr(self, expr: AwaitExpr) -> None:
        if not self.is_func_scope() or not self.function_stack:
            ...
            self.fail(
                '"await" outside function', expr, serious=True, code=codes.TOP_LEVEL_AWAIT
            )
    ...
    def expr_to_analyzed_type(
        ...
        if unique_name is not None and isinstance(expr, CallExpr):
            ...
            if tvar_defs:
                self.fail(
                    "Generic named tuples are not supported for legacy class syntax", expr
                )
