...
def compile_new_format_re(custom_spec: bool) -> Pattern[str]:
    ...
    if not custom_spec:
        ...
        conv_type = (
            r"(?P<type>.)?"  # only some are supported, but we want to give a better error
        )
...
def parse_format_value(
    ...
    for target, start_pos in top_targets:
        ...
        if conv_spec.key and ("{" in conv_spec.key or "}" in conv_spec.key):
            msg.fail(
                "Conversion value must not contain { or }", ctx, code=codes.STRING_FORMATTING
            )
            return None
    ...
    def check_simple_str_interpolation(
        ...
        if isinstance(rhs_type, TupleType):
            ...
            if unpack_index is not None:
                ...
                rep_types = (
                    rep_types[:unpack_index] + unpack_items + rep_types[unpack_index + 1 :]
                )
