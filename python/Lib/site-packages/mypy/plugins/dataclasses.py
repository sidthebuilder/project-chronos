...
class DataclassAttribute:
    ...
    def to_argument(
        ...
        if of == "__init__":
            ...
            arg_kind = (
                ARG_NAMED if self.is_init_var and not self.has_default else ARG_NAMED_OPT
            )
...
class DataclassTransformer:
        ...
        if (
            ...
            if info.fallback_to_any:
                ...
                args = [
                    ...
                    *args,
                    Argument(
                        Var(gen_kwargs_name), AnyType(TypeOfAny.explicit), None, ARG_STAR2
                    ),
                ]
    ...
    def _add_dataclass_fields_magic_attribute(self) -> None:
        ...
        if self._spec is _TRANSFORM_SPEC_FOR_DATACLASSES:
            field_type = (
                self._api.named_type_or_none("dataclasses.Field", [any_type]) or any_type
            )
        else:
            field_type = any_type
...
def replace_function_sig_callback(ctx: FunctionSigContext) -> CallableType:
    ...
    if len(ctx.args) != 2:
        ...
        ctx.api.fail(
            f'"{ctx.default_signature.name}" has unexpected type annotation', ctx.context
        )
