...
class CParser:
    ...
    def _add_declaration_specifier(
        ...
        if declspec is None:
            spec: _DeclSpec = dict(qual=[], storage=[], type=[], function=[], alignment=[])
        else:
            spec = declspec
    ...
    def _build_declarations(
        ...
        for decl in decls:
            ...
            fixed_decl = fix_atomic_specifiers(cast(c_ast.Decl | c_ast.Typedef, fixed_decl))
    ...
    def _parse_external_declaration(self) -> List[c_ast.Node]:
        ...
        spec, saw_type, spec_coord = self._parse_declaration_specifiers(allow_no_type=True)
    ...
    def _parse_init_declarator_list(
        ...
        decls = (
            [first] if first is not None else [self._parse_init_declarator(id_only=id_only)]
        )
    ...
    def _parse_struct_or_union_specifier(self) -> c_ast.Node:
        ...
        if self._peek_type() in {"ID", "TYPEID"}:
            ...
            if self._peek_type() == "LBRACE":
                ...
                self._expect("RBRACE")
                return klass(name=name_tok.value, decls=decls, coord=self._tok_coord(name_tok))

            return klass(name=name_tok.value, decls=None, coord=self._tok_coord(name_tok))
    ...
    #                        | direct_declarator '[' ... ']'
    #                        | direct_declarator '(' ... ')'
    def _parse_direct_declarator(self, kind: str, allow_paren: bool = True) -> c_ast.Node:
    ...
    def _parse_array_decl_common(
        ...
        def make_array_decl(dim, dim_quals):
            return c_ast.ArrayDecl(type=base_type, dim=dim, dim_quals=dim_quals, coord=coord)
    ...
    def _parse_primary_expression(self) -> c_ast.Node:
        ...
        if tok_type in _INT_CONST or tok_type in _FLOAT_CONST or tok_type in _CHAR_CONST:
...
_STARTS_EXPRESSION = (
    _EXPR_START | _INT_CONST | _FLOAT_CONST | _CHAR_CONST | _STRING_LITERAL | _WSTR_LITERAL
)
