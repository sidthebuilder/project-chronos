...
def test_dicttoolz():
    ...
    assert raises(TypeError, lambda: itemmap(None, {1: 2})) or itemmap(None, {1: 2}) == {
        1: (2,)
    }
...
def test_itertoolz():
    ...
    # XXX
    assert raises(TypeError, lambda: list(drop(None, [1, 2]))) or list(drop(None, [1, 2])) == [
        1,
        2,
    ]
    ...
    assert raises(
        TypeError,
        lambda: list(interleave([[1, 2], GenException(ValueError)], pass_exceptions=None)),
    )
...
def test_recipes():
    tested = []
    # XXX
    assert raises(TypeError, lambda: countby(None, [1, 2])) or countby(None, [1, 2]) == {
        (1,): 1,
        (2,): 1,
    }
