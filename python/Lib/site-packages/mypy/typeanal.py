...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
    ...
    def try_analyze_special_unbound_type(self, t: UnboundType, fullname: str) -> Type | None:
        ...
        if fullname == "builtins.None":
            ...
            if bad_item_name:
                self.fail(
                    f'{type_str} can\'t contain "{bad_item_name}"', t, code=codes.VALID_TYPE
                )
    ...
    def visit_callable_type(
        ...
        with self.tvar_scope_frame(namespace=namespace):
            ...
            type_guard = (
                self.anal_type_guard(t.ret_type) if t.type_guard is None else t.type_guard
            )
            ...
            ret = t.copy_modified(
                ...
                fallback=(
                    t.fallback if t.fallback.type else self.named_type("builtins.function")
                ),
    ...
    def visit_tuple_type(self, t: TupleType) -> Type:
        ...
        if t.implicit and not self.allow_tuple_literal:
            ...
            if len(t.items) == 0:
                ...
                self.note(
                    "Suggestion: Is there a spurious trailing comma?", t, code=codes.SYNTAX
                )
            else:
                self.note(
            ...
            for typ in t.extra_items_from:
                ...
                if not isinstance(p_analyzed, TypedDictType):
                    if not isinstance(p_analyzed, (AnyType, PlaceholderType)):
                        self.fail(
                            "Can only merge-in other TypedDict", t, code=codes.VALID_TYPE
                        )
                    continue
    ...
    def analyze_callable_args(
        ...
        if seen_unpack:
            if len(unpack_types) == 1:
                ...
                args.append(
                    UnpackType(
                        self.tuple_type(unpack_types, line=first.line, column=first.column)
                    )
                )
    ...
    def analyze_literal_type(self, t: UnboundType) -> Type:
        if len(t.args) == 0:
            self.fail(
                "Literal[...] must have at least one parameter", t, code=codes.VALID_TYPE
            )
            return AnyType(TypeOfAny.from_error)
    ...
    def analyze_literal_param(self, idx: int, arg: Type, ctx: Context) -> list[Type] | None:
        ...
        if isinstance(arg, AnyType):
            ...
            self.fail(
                f"Parameter {idx} of Literal[...] is invalid", ctx, code=codes.VALID_TYPE
            )
            return None
    ...
    def check_unpacks_in_list(self, items: list[Type]) -> list[Type]:
        ...
        if num_unpacks > 1:
            ...
            self.fail(
                "More than one variadic Unpack in a type is not allowed", final_unpack.type
            )
        return new_items
