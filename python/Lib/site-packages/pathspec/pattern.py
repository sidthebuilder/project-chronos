...
class RegexPattern(Pattern):
    ...
    def __init__(
        ...
        *pattern* is a precompiled regular expression (:class:`re.Pattern`) in which
        ...
        if isinstance(pattern, (str, bytes)):
            assert include is None, f"{include=!r} must be null when {pattern=!r} is a string."
            ...
            # operation.
            assert include is None, f"{include=!r} must be null when {pattern=!r} is null."
    ...
    def match_file(self, file: AnyStr) -> Optional[RegexMatchResult]:
        ...
        *file* (:class:`str` or :class:`bytes`) is the file path relative to the
        ...
        otherwise, :data:`None`.
        """
        if (regex := self.regex) is not None and (match := regex.search(file)) is not None:
