...
class MessageBuilder:
    ...
    def has_no_attr(
        ...
        if isinstance(original_type, Instance) and original_type.type.has_readable_member(
            member
        ):
    ...
    def not_callable(self, typ: Type, context: Context) -> Type:
        self.fail(
            message_registry.NOT_CALLABLE.format(format_type(typ, self.options)), context
        )
        ...
        if callee_name is not None:
            ...
            if name.startswith('"__getitem__" of'):
                ...
                if n == 1:
                    ...
                    error_msg = (
                        message_registry.INCOMPATIBLE_TYPES_IN_ASSIGNMENT.with_additional_msg(
                            info
                        )
                    )
    ...
    def does_not_return_value(self, callee_type: Type | None, context: Context) -> None:
        ...
        callee_name = (
            callable_name(callee_type) if isinstance(callee_type, FunctionLike) else None
        )
    ...
    def no_variant_matches_arguments(
        ...
        *,
        ...
        self.note(
            f"Possible overload variant{plural_s(len(overload.items))}:", context, code=code
        )
        for item in overload.items:
    ...
    def signature_incompatible_with_supertype(
        ...
        *,
        ...
        if isinstance(original, (CallableType, Overloaded)):
            ...
            self.note(
                original_str, context, offset=ALIGN_OFFSET + 2 * OFFSET, parent_error=error
            )
        ...
        if isinstance(override, (CallableType, Overloaded)):
            ...
            self.note(
                override_str, context, offset=ALIGN_OFFSET + 2 * OFFSET, parent_error=error
            )
    ...
    def var_used_before_def(self, name: str, context: Context) -> None:
        self.fail(
            f'Name "{name}" is used before definition', context, code=codes.USED_BEFORE_DEF
        )
    ...
    def too_few_string_formatting_arguments(self, context: Context) -> None:
        self.fail(
            "Not enough arguments for format string", context, code=codes.STRING_FORMATTING
        )
    ...
    def type_arguments_not_allowed(self, context: Context) -> None:
        self.fail(
            "Parameterized generics cannot be used with class or instance checks", context
        )
    ...
    def disallowed_any_type(self, typ: Type, context: Context) -> None:
        ...
        if isinstance(typ, AnyType):
            ...
            message = (
                f'Expression type contains "Any" (has type {format_type(typ, self.options)})'
            )
    ...
    def report_protocol_problems(
        ...
        *,
        ...
        if (
            missing
            and (
                len(missing) < len(supertype.type.protocol_members) or missing == ["__call__"]
            )
            ...
            if missing == ["__call__"] and class_obj:
                ...
                self.note(
                    ", ".join(missing), context, offset=OFFSET, parent_error=parent_error
                )
...
def best_matches(current: str, options: Collection[str], n: int) -> list[str]:
    ...
    ratios = {
        option: difflib.SequenceMatcher(a=current, b=option).ratio() for option in options
    }
