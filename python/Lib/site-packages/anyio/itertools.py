...
class Chain:
    def __call__(self, *iterables: Iterable[T] | AsyncIterable[T]) -> AsyncGenerator[T, None]:
...
async def islice(
    ...
    *args: int | None,
    ...
    start_message = "Indices for islice() must be None or an integer: 0 <= x <= sys.maxsize."
    stop_message = (
    ...
    start = 0 if slice_args.start is None else normalize_index(slice_args.start, start_message)
    stop = None if slice_args.stop is None else normalize_index(slice_args.stop, stop_message)
    step = 1 if slice_args.step is None else normalize_index(slice_args.step, step_message)
...
def tee(iterable: Iterable[T] | AsyncIterable[T], n: int = 2) -> tuple[AsyncIterator[T], ...]:
