...
class StubtestUnit(unittest.TestCase):
    ...
    def test_coroutines(self) -> Iterator[Case]:
        yield Case(
            stub="def bar() -> int: ...", runtime="async def bar(): return 5", error="bar"
        )
        # Don't error for this one -- we get false positives otherwise
        yield Case(
            stub="async def foo() -> int: ...", runtime="def foo(): return 5", error=None
        )
    ...
    def test_varargs_varkwargs(self) -> Iterator[Case]:
        ...
        yield Case(
            stub="def k1(a, **kwargs) -> None: ...", runtime="def k1(a): pass", error="k1"
        )
        yield Case(
    ...
    def test_type_alias(self) -> Iterator[Case]:
        ...
        yield Case(
            stub="StrList = list[str]", runtime="StrList = ['foo', 'bar']", error="StrList"
        )
        yield Case(
            stub="""
...
""",
    ...
    def test_metaclass_abcmeta(self) -> Iterator[Case]:
        ...
        yield Case(
            stub="from abc import ABCMeta", runtime="from abc import ABCMeta", error=None
        )
        yield Case(
