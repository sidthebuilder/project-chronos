...
def fixture(
    ...
    *,
    ...
    autouse: bool = ...,
    ids: (
        Iterable[str | float | int | bool | None] | Callable[[Any], object | None] | None
    ) = ...,
    name: str | None = ...,
...
def _wrap_syncgen_fixture(
    fixture_function: Callable[SyncGenFixtureParams, Generator[SyncGenFixtureYieldType]],
    runner: Runner,
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    specialized_item_class = PytestAsyncioFunction.item_subclass_for(metafunc.definition)
    ...
    if asyncio_marker is None:
        return
    marker_loop_scope, marker_selected_factory_names = _parse_asyncio_marker(asyncio_marker)
...
def pytest_fixture_setup(fixturedef: FixtureDef, request) -> object | None:
    if fixturedef.argname == "event_loop_policy" and fixturedef.func.__module__ != __name__:
        warnings.warn(
    ...
    runner_fixturedef.addfinalizer(functools.partial(fixturedef.finish, request=request))
...
def _parse_asyncio_marker(
    ...
    if not marker_value or any(
        not isinstance(factory_name, str) or not factory_name for factory_name in marker_value
    ):
        raise ValueError(_INVALID_LOOP_FACTORIES_KWARG)
...
def _validate_asyncio_marker(asyncio_marker: Mark) -> None:
    if asyncio_marker.args or (
        ...
        msg = (
            "mark.asyncio accepts only keyword arguments 'loop_scope' and" " 'loop_factories'."
        )
        raise ValueError(msg)
