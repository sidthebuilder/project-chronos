...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi, SplittingVisitor):
    ...
    def get_generator_receive_type(self, return_type: Type, is_coroutine: bool) -> Type:
        ...
        if isinstance(return_type, AnyType):
            ...
            return_type.type.fullname == "typing.AsyncGenerator" and len(return_type.args) >= 2
        ):
            return return_type.args[1]
        else:
            ...
            if (
                ...
                for i, (sub_kind, super_kind) in enumerate(
                    ...
                    if sub_kind.is_positional() and super_kind.is_positional():
                        ...
                        sub_kind.is_named() and super_kind.is_named() and not offset_arguments
                    ):
            ...
            if isinstance(lvalue, NameExpr) and lvalue.node:
                ...
                if name == "__post_init__":
                    ...
                    if active_class and dataclasses_plugin.is_processed_dataclass(
                        ...
                        self.fail(
                            message_registry.DATACLASS_POST_INIT_MUST_BE_A_FUNCTION, rvalue
                        )
    ...
    def check_compatibility_all_supers(self, lvalue: RefExpr, rvalue: Expression) -> None:
        ...
        if (
            ...
            if lvalue_node.is_inferred and not lvalue_node.explicit_self_type:
                ...
                ctx = (
                    lvalue_node.type if not isinstance(lvalue_node.type, PartialType) else None
                )
            ...
            for idx_rval, rval in enumerate(self.flatten_rvalues(rvalue.items)):
                if isinstance(rval, StarExpr):
                    ...
                    if self.type_is_iterable(typs) and isinstance(typs, Instance):
                        ...
                        self.fail(
                            message_registry.ITERABLE_TYPE_EXPECTED.format(typs), context
                        )
                else:
                    rvalues.append(rval)
    ...
    def check_for_truthy_type(self, t: Type, expr: Expression) -> None:
        ...
        if isinstance(t, FunctionLike):
            ...
            self.fail(
                message_registry.TYPE_ALWAYS_TRUE_UNIONTYPE.format(format_expr_type()), expr
            )
                    ...
                    if node.arg_kinds[0] != nodes.ARG_POS:
                        ...
                        if isinstance(called_type, (CallableType, Overloaded)):
                            ...
                            if name in node.arg_names:
                                ...
                                self.fail(
                                    message_registry.TYPE_GUARD_POS_ARG_REQUIRED.format(kind),
                                    node,
                                )
                                return {}, {}
                    if literal(expr) == LITERAL_TYPE:
